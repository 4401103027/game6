...
class scene2 extends Phaser.Scene{
	...
	create(){
		...
		this.add.image(600, 100, 'label1');
		this.add.image(600, 360, 'keyboard');

		// HIỆN PHÍM CẦN GÕ
		var x_key = 640, y_key = 393;
		var key = {}
		for (var i in keys) {
		...
		// KÝ TỰ
		var str = list[list_index]
		var x_char = 470, y_char = 100;
		var char = []
		for (var i in str) {
			char.push(this.add.image(x_char, y_char, str[i] + '1'));
			x_char += 88;
		}

		var index = 0;
		var current_char = str[index]
		var current_line = this.add.image(char[index].x, char[index].y + 45, 'line1');
		...
		this.input.keyboard.on('keydown', function (event) {
			...
			if (event.keyCode === str[index].toUpperCase().charCodeAt(0)|| event.keyCode == 188 || event.keyCode == 190 || event.keyCode == 191 )
			{
				this.add.image(char[index].x, char[index].y, current_char + '2');
				...
				if (index == str.length) {
					...
					current_line = this.add.image(char[index].x, char[index].y + 45, 'line1');
					this.add.image(char[index].x - 25, char[index].y - 33, 'star1')
					key[current_char].visible = true;
					this.bee_finger(current_char);
				}
